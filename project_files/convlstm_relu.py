...
def cnnlstm_model(sequence_length=10,
    ...
    x = Convolution1D(filters=64, kernel_size=3, activation=activation, padding='valid',
                      kernel_initializer=init, input_shape=(sequence_length, 1), name='conv1')(inputs)
    x = BatchNormalization()(x)
    x = Convolution1D(filters=32, kernel_size=filter_length, activation=activation, padding='valid',
                      kernel_initializer=init, input_shape=(sequence_length, 1), name='conv2')(x)
    x = BatchNormalization()(x)
    x = MaxPool1D(pool_size=2)(x)
    x = Convolution1D(filters=16, kernel_size=filter_length, activation=activation, padding='valid',
                      kernel_initializer=init, input_shape=(sequence_length, 1), name='conv3')(x)
    x = BatchNormalization()(x)
    x = Convolution1D(filters=16, kernel_size=filter_length, activation=activation, padding='valid',
                      kernel_initializer=init, input_shape=(sequence_length, 1), name='conv4')(x)
    x = BatchNormalization()(x)
